...
if __name__ == "__main__":
    filename = "comm.txt" # Change filename for whatever your file is called
    file = open(filename, "r")
    namenpass = file.read()
    namenpass2 = namenpass.split(" ")
    print(namenpass2)
    file.close()
    
    file = open(filename, "w")
    err = checkUser(namenpass2[0], namenpass2[1])
    if err == 102:
        resp = "Error with your input, please try again and remember the system is case sensitive."
    elif err == 101:
        print("success")
        resp = exampleFunc(namenpass2[0])
